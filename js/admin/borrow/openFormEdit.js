$(document).on("click", ".list__action-open-edit", function () {
    var id = $(this).data('id');
    var user = $(this).closest("tr").find("td:eq(1)").text().trim();
    var book = $(this).closest("tr").find("td:eq(2)").text().trim();
    var status = $(this).closest("tr").find("td:eq(3)").text().trim();
    var timeRequest = $(this).closest("tr").find("td:eq(4)").text().trim();
    var timeBorrow = $(this).closest("tr").find("td:eq(5)").text().trim();
    var timeReturn = $(this).closest("tr").find("td:eq(6)").text().trim();
   ...
   if (listBook.length > 0) {
        for (var i = 0; i < listBook.length; i++) {
            ...
            optionBook += '<option value="' + listBook[i][0] + '" selected>'+ listBook[i][1] + '</option>';
            }else{
                optionBook += '<option value="' + listBook[i][0] + '">'+ listBook[i][1] + '</option>';
            }
        } 
    }

    if(status === 'Đang mượn'){
        newTime = '<button name="new_time">Gia hạn</button>';
    }else{
        newTime = '';
    }
    ...
    if (listUser.length > 0) {
        for (var i = 0; i < listUser.length; i++) {
            if(listUser[i][1] === user){
                optionUser += '<option value="' + listUser[i][0] + '" selected>' + listUser[i][1] + ' - ' + listUser[i][3] + "</option>";
            }else{
                optionUser += '<option value="' + listUser[i][0] + '">' + listUser[i][1] + ' - ' + listUser[i][3] + "</option>";
            }   
        }
    }
    ...
    if(selectStatus.options.length>1){
        for (var i = 1; i < selectStatus.options.length; i++) {
            ...
            if(option.text === status){
                optionSelectStatus += '<option value="' + option.value + '" selected>' + option.text + '</option>';
            }else{
                optionSelectStatus += '<option value="' + option.value + '">' + option.text + '</option>';
            }
          }
    }
