...
if (!empty($jsonObj->request_type) && $jsonObj->request_type == 'user_auth') {
    ...
    if (!empty($responsePayload)) {
        ...
        $query = "SELECT * FROM google_account WHERE oauth_provider = '" . $oauth_provider . "' AND oauth_uid = '" . $oauth_uid . "'";
        $result = $db->query($query);

        if ($result->num_rows > 0) {
            // Update user data if already exists 
            $query = "UPDATE google_account SET first_name = '" . $first_name . "', last_name = '" . $last_name . "', email = '" . $email . "', picture = '" . $picture . "', modified = NOW() WHERE oauth_provider = '" . $oauth_provider . "' AND oauth_uid = '" . $oauth_uid . "'";
            $update = $db->query($query);
        } else {
            // Insert user data 
            $query = "INSERT INTO google_account VALUES (NULL, '" . $oauth_provider . "', '" . $oauth_uid . "', '" . $first_name . "', '" . $last_name . "', '" . $email . "', '" . $picture . "', NOW(), NOW())";
